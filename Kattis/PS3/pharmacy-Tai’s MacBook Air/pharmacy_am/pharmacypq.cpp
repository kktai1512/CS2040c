...
int main() {
    ...
    for (ll T = 1; T < LLONG_MAX; T ++) {
        ...
        if (!time_list.empty()){
                ...
                while (-time_list.top() < T and !time_list.empty()) {
                    time_list.pop();
                }
                
                ll min_time = 0;
                if (!time_list.empty()){
                        min_time= -time_list.top();
                }
                cout << "min time: " << min_time << endl;
                ll target = min(min_time, min_pres);
                ...
                if (min_time == 0) {
                    cout << "we jump to baa " << min_pres << endl;
                    T = min_pres -1;
                }
        }
        
        cout << "we jump to b " << min_pres << endl;
        T = min_pres == 0? T : min_pres -1;
    }
