...
int main() {
    ...
    ll counter = 0;
    ll most_recent_push = 0;
    priority_queue<ll> time_list;
    for (ll T = 1; T < LLONG_MAX; T ++) {
        ...
        //     break;
        // }
        if (in_store_size + remote_size == prescriptions.size() ) {
            break;
        }
        ll k = 0;
        ...
        counter += k;
        triplet min_in_prescription = prescriptions[counter];
        ll min_pres = min_in_prescription.dtime;
      
        if (min_pres!= 0) {
            if (-min_pres != most_recent_push){
                time_list.push(-min_pres);
                most_recent_push = -min_pres;
            }
        }
        ...
        for (ll n_tech = 0; n_tech < t; n_tech ++) {
            ...
            if (!queue.empty() && queue.top().dtime <= T) {
                if (avail_tech[n_tech].first == true) {
                    ...
                    if (-completed_time != most_recent_push){
                        time_list.push(-completed_time);
                        most_recent_push = -completed_time;
                    }

                    if (queue.top().type == 'S') {
                        //in_store_time.push_back(completed_time - queue.top().dtime);
                        in_store_total += (completed_time - queue.top().dtime);
                        in_store_size +=1;
                    }
                    if (queue.top().type == 'R') {
                        //remote_time.push_back(completed_time - queue.top().dtime);
                        remote_time_total += (completed_time - queue.top().dtime);
                        remote_size += 1;
                    }
                    queue.pop();
                }
        ...
        if (!time_list.empty()){
            ll temp = -time_list.top() -1;
            if (T< temp) {
                T = temp;
            }
            //cout << "jumping to " << T+1 << endl;
            time_list.pop();
        }
    ...
    float in_store_average = in_store_size == 0? 0 : (float)in_store_total/in_store_size;
    float remote_average = remote_size == 0? 0 : (float)remote_time_total/remote_size;
